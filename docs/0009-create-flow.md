...
> 模块：create-flow ｜ 优先级：9 ｜ 依赖：0007,0008,0006 ｜ 里程碑：M1
> 对应 SPEC：「创作页」
...
## 涉及文件
- `src/app/(tabs)/index.tsx`（创作页主体）
- `src/features/create/PresetGrid.tsx`、`ChipsSelector.tsx`、`IdeaInput.tsx`、`GenerateButton.tsx`、`ResultView.tsx`
- `src/features/create/useGenerate.ts`（封装 `POST /generate` + `GET /jobs/:id` 轮询）

## 实现要点
- 用 `@expo/ui` 原生组件 + `expo-image`；`useGenerate` 基于 React Query（mutation 建任务 + query 轮询直到 succeeded/failed）。
