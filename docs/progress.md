...
> 拆分原则：每个模块尽量**独立、可单独测试**；编号越小优先级越高（从 0001 起）。

## 使用方式
- 每个模块有独立规格文件 `000x-<module>.md`，含「目标 / 范围 / 涉及文件 / 实现要点 / 独立测试 / 完成标准」。
...
## 依赖与并行建议
- **关键路径（先做）**：0001 → 0002/0003/0004（可并行）→ Codex 图片能力 spike → 0005 → 0006。完成后后端核心出图链路即可独立验证。
