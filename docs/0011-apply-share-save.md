...
> 模块：apply-share-save ｜ 优先级：11 ｜ 依赖：0010,0009 ｜ 里程碑：M2
> 对应 SPEC：「应用/分享」
...
## 涉及文件
- `src/features/apply/ApplySheet.tsx`（底部操作：应用/分享/保存 + target 选择）
- `src/features/apply/useApplyWallpaper.ts`（先下载远程图到本地 → 调 `expo-wallpaper`）
- `src/features/apply/useSaveAndShare.ts`
...
## 独立测试
- Android dev build：出一张图 → ApplySheet 选「两者」→ 桌面/锁屏确认更换；再测「存相册」（相册可见）与「分享」（面板弹出）。
