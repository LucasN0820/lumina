...
## 范围
- In：图节点 `resolvePreset -> enrichPrompt(可开关) -> route -> generate/edit -> persist`，以及 `runWallpaperGraph(input): Promise<Wallpaper>`。
- Out：HTTP 任务封装（0006）、完整编辑 UI（0015）。

## 涉及文件
- `server/src/graph/wallpaper.graph.ts`（图定义 + 编译）
- `server/src/graph/nodes/*.ts`（各节点）
- `server/src/graph/state.ts`（图状态类型）
...
- 节点职责：
  1. `resolvePreset`：读 `Preset`（0002）+ 注入用户 chips/想法 + 设备 W x H -> 填好 `prompt/negativePrompt/width/height`。
  2. `enrichPrompt`：用 Codex/OpenAI 文本能力把「一句话」扩写为专业 image prompt（env 开关 `ENRICH_PROMPT`，关则跳过省额度）。
...
- **合规占位**：在 `route` 前 / `persist` 前留 `// TODO: moderation / watermark / policy checks`，本期不实现。
- 失败处理：任一节点抛错 -> 捕获 -> `Wallpaper.status=failed` + `error`。
- Codex 额度防护：记录 job 开始/结束、失败原因、耗时；额度耗尽或达到本地限流时返回可理解错误。

## 独立测试
- 脚本 `server/scripts/try-graph.ts`：构造 `{presetId, userInputs, width:1080, height:2400, mode:'text2img'}` -> `runWallpaperGraph` -> 断言返回 `Wallpaper.status==='succeeded'` 且 `resultImageUrl` 可访问、尺寸接近目标。
